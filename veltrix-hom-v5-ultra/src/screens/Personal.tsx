...
export default function Personal() {
  ...
  useEffect(() => {
    activityApi.summary().then(setSummary).catch(() => setSummary(EMPTY)).finally(() => setLoading(false))
  }, [])
    ...
    <section className="v5-personal-page">
      <div className="v5-personal-inner" style={{ display: 'grid', gap: 16 }}>
        ...
        <div className="v5-modes-grid">
          ...
          <motion.button className="v5-mode-card" style={{ '--mode-color': '#8B5CF6' } as React.CSSProperties} whileTap={{ scale: .975 }} onClick={() => navigate('/skills')}>
            <span className="v5-source-icon" style={{ '--source-color': '#8B5CF6', width: 48, height: 48 } as React.CSSProperties}><Sparkles size={22}/></span>
            <strong style={{ display: 'block', marginTop: 15, fontSize: 17 }}>Skills</strong><span className="micro clamp-2" style={{ display: 'block', marginTop: 5 }}>Claude Skills kabi AI yo‘riqnomalari.</span>
          </motion.button>
        </div>
      </div>
    </section>

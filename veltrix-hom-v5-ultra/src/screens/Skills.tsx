  ...
  return (
    <div data-scroll-root className="hide-sb"
      ...
      <div style={{
        ...
        {list.map((s, index) => (
          <motion.article key={s.id} className="v5-mode-card" style={{ '--mode-color': s.color || '#8B5CF6', minHeight: 185 } as React.CSSProperties}
            initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(index * .035, .2) }}>
            <div className="row" style={{ gap: 11, alignItems: 'flex-start' }}>
              <span aria-hidden className="v5-source-icon" style={{ '--source-color': s.color || '#8B5CF6', width: 48, height: 48 } as React.CSSProperties}>{s.emoji}</span>
      ...
      <AnimatePresence>
        ...
        {editing && (
          <SkillEditor
            ...
            subjects={subjects}
            onClose={() => { setEditing(null); if (params.has('add')) { params.delete('add'); setParams(params, { replace: true }) } }}
          />
        )}
      </AnimatePresence>
    </div>

...
insert into public.vh_accounts(id,email) values(:'a'::uuid,'stage90-perf-a@example.invalid'),(:'b'::uuid,'stage90-perf-b@example.invalid');
insert into public.vh_conversations(account_id,title,last_message_at)
 select :'a'::uuid,'needle performance conversation '||g,now()-g*interval '1 second' from generate_series(1,500) g
 union all select :'b'::uuid,'owner b conversation '||g,now()-g*interval '1 second' from generate_series(1,100) g;
insert into public.vh_conversation_messages(account_id,conversation_id,role,status,plain_text,content_blocks,completed_at)
 ...
 from public.vh_conversations c cross join lateral generate_series(1,case when c.account_id=:'a'::uuid then 40 else 20 end) g
 where c.account_id in (:'a'::uuid,:'b'::uuid);
with s as (select public.vh_begin_fast_ask(:'a'::uuid,'perf-fast','performance prompt','{}') j),
 ids as (select (j->>'fastAskId')::uuid id,(j->>'requestId')::uuid request from s)
select public.vh_complete_fast_ask(:'a'::uuid,id,request,'done',jsonb_build_array(jsonb_build_object('id','answer-1','type','answer','version',1,'text','done')),'{}','{}') from ids;
insert into public.vh_fast_ask_stream_events(account_id,fast_ask_id,request_id,seq,event_type,payload)
 select :'a'::uuid,s.id,s.request_id,g+1,'block.delta',jsonb_build_object('delta','x')
 from public.vh_fast_ask_sessions s cross join generate_series(1,2000) g where s.account_id=:'a'::uuid and s.idempotency_key='perf-fast';
with r as (select public.vh_begin_tool_run(:'a'::uuid,'calculator','perf-tool',jsonb_build_object('expression','2+2'),'{}',120) j)
select public.vh_complete_tool_run(:'a'::uuid,(j->>'toolRunId')::uuid,(j->>'claimToken')::uuid,jsonb_build_object('kind','calculator','result',4),'{}','{}') from r;
insert into public.vh_tool_runs(account_id,tool_type,status,input_payload,output_payload,completed_at)
 select :'a'::uuid,'calculator','COMPLETED',jsonb_build_object('expression',g||'+1'),jsonb_build_object('result',g+1),now() from generate_series(1,999) g;
...
latency_gate() { local name=$1 ceiling=$2 sql=$3; local file=/tmp/"$name".samples; : >"$file"; measure "$sql" /dev/null; measure "$sql" /dev/null; for _ in {1..10}; do measure "$sql" "$file"; done; local value; value=$(p95 "$file"); echo "PERF_SAMPLES op=$name values=$(paste -sd, "$file") p95_ms=$value ceiling_ms=$ceiling"; awk -v v="$value" -v c="$ceiling" 'BEGIN{exit !(v<=c)}'; }
...
concurrency_gate() { local name=$1 sql=$2; local file=/tmp/"$name".samples rates=/tmp/"$name".rates; : >"$file"; : >"$rates"; for warm in 1 2; do run_sql "$sql"; done; for batch in {1..5}; do local bs be rate; bs=$(date +%s%N); for i in {1..16}; do measure "$sql" "$file" & done; wait; be=$(date +%s%N); rate=$(awk -v n="$((be-bs))" 'BEGIN{printf "%.3f",16/(n/1e9)}'); echo "$rate" >>"$rates"; awk -v n="$((be-bs))" -v b="$batch" -v r="$rate" 'BEGIN{printf "PERF_BATCH batch=%d size=16 elapsed_ms=%.3f throughput=%.3f\n",b,n/1e6,r}'; done; local value min_tput; value=$(p95 "$file"); min_tput=$(sort -n "$rates" | head -1); echo "PERF_SAMPLES op=$name count=$(wc -l <"$file") p95_ms=$value ceiling_ms=500 min_throughput=$min_tput floor=25"; awk -v v="$value" -v t="$min_tput" 'BEGIN{exit !(v<=500 && t>=25)}'; }

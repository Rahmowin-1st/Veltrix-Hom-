...
router.post('/library/upload-ticket', async (req, res, next) => {
  try {
    ...
    try {
      ...
      const { data: object, error: objectError } = await admin.from('vh_storage_objects').insert({ account_id: id, bucket: 'vh-library', object_path: path, kind: 'library', mime_type: parsed.mimeType, size_bytes: parsed.sizeBytes, state: 'pending' }).select('id').single()
...
router.post('/library/upload-commit', async (req, res, next) => {
  try {
    ...
    const id = accountId(req)
    const { data: object, error: objectError } = await admin.from('vh_storage_objects').select('id,bucket,object_path,size_bytes,state').eq('id', parsed.objectId).eq('account_id', id).eq('kind', 'library').single()
    if (objectError) throw objectError
    if (object.state !== 'pending') throw new ApiError(409, 'UPLOAD_NOT_PENDING', 'The upload is not pending.')
    const { data: reservation, error: reservationError } = await admin.from('vh_quota_reservations').select('id,account_id,scope,bytes,status,expires_at').eq('id', parsed.reservationId).eq('account_id', id).eq('scope', 'library').single()
    if (reservationError) throw reservationError
    if (reservation.status !== 'pending' || Date.parse(reservation.expires_at) <= Date.now()) throw new ApiError(409, 'RESERVATION_EXPIRED', 'The storage reservation expired.')
    if (Number(reservation.bytes) !== Number(object.size_bytes)) throw new ApiError(409, 'RESERVATION_MISMATCH', 'The storage reservation does not match the upload.')
    ...
    const { error: readyError } = await admin.from('vh_storage_objects').update({ state: 'ready', updated_at: new Date().toISOString() }).eq('id', object.id).eq('account_id', id)
...
router.delete('/library/:objectId', async (req, res, next) => {
  try {
    ...
    const { data, error } = await admin.from('vh_storage_objects').update({ state: 'trashed', trashed_at: now.toISOString(), purge_after: purgeAfter.toISOString(), updated_at: now.toISOString() }).eq('id', objectId).eq('account_id', id).eq('kind', 'library').neq('state', 'trashed').select('id').maybeSingle()
    if (error) throw error
    if (!data) throw new ApiError(404, 'OBJECT_NOT_FOUND', 'The object was not found.')

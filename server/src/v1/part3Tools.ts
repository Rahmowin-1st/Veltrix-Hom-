...
export const helpSolveOutputSchema = z.object({
  kind: z.literal('solve'),
  mode: z.literal('HELP_ME_SOLVE'),
  problemType: problemTypeSchema,
  simplifiedTask: z.string().min(1).max(5000),
  whatIsAsked: z.string().min(1).max(3000),
  givens: z.array(z.string().min(1).max(2000)).max(30),
  difficultPoint: z.string().min(1).max(3000),
  principle: z.string().min(1).max(5000),
  startGuidance: z.string().min(1).max(5000),
  nextStepGuidance: z.string().min(1).max(5000),
  hints: z.array(z.string().min(1).max(3000)).min(1).max(12),
  inputReference: inputReferenceSchema,
}).strict()
...
export const summarizeOutputSchema = z.object({
  ...
  keyPoints: z.array(z.string().min(1).max(3000)).max(30),
  sourceReferences: z.array(sourceReferenceSchema).max(50),
}).strict()

export const toolOutputSchema = z.discriminatedUnion('kind', [
  calculatorOutputSchema,translateOutputSchema,solveOutputSchema,summarizeOutputSchema,
])
...
async function completeToolRun(id: string, run: z.infer<typeof beginToolRunResultSchema>, output: z.infer<typeof toolOutputSchema>, modelRoute: Record<string, unknown>, provenance: Record<string, unknown>) {
...
async function getAssetContext(id: string, assetIds: string[]) {
  ...
  const references = selected.map(row => sourceReferenceSchema.parse({ assetId: row.asset_id,title: row.display_title,locator: row.locator,contentHash: row.content_hash }))
...
export function assertHelpModeNoFinalAnswer(output: z.infer<typeof helpSolveOutputSchema>) {
  const guidance = [output.difficultPoint,output.principle,output.startGuidance,output.nextStepGuidance,...output.hints].join('\n')
  const forbidden = [
...
async function runSolve(runId: string, input: z.infer<typeof solveInputSchema>, sourceContext: string) {
  const sourceKinds = sourceContext ? 'Library source context is included below.' : 'No Library source context.'
...
async function replayOrConflict(id: string, run: z.infer<typeof beginToolRunResultSchema>) {
  ...
  if (!run.authoritative) throw new ApiError(202, 'TOOL_RUN_IN_PROGRESS', 'The same tool request is already running.')
  return null
}
...
router.post('/tools/calculator', async (req, res, next) => {
  ...
  try {
    ...
    const replay = await replayOrConflict(id, run)

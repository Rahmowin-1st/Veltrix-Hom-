...
export async function deliverQueuedNotification(account: string, notificationId: string, signal?: AbortSignal, providers = pushProviders) {
  ...
  for (const tokenRow of tokens) {
    ...
    const key = { id: notificationId, account_id: account }
    const deliveryKey = { notification_id: notificationId, device_token_id: tokenRow.id, account_id: account }
    ...
    if (!provider) {
      ...
      const { error: deliveryUpdateError } = await admin.from('vh_notification_deliveries')
        ...
        .eq('account_id', deliveryKey.account_id).eq('notification_id', deliveryKey.notification_id).eq('device_token_id', deliveryKey.device_token_id)
  ...
  // A concurrent/replayed worker that claimed nothing must not race the true sender's
  // notification state. It simply observes the current state and exits.
  if (sent === 0 && failed === 0) {
  ...
  let update = admin.from('vh_notifications').update({ outside_state: finalState, outside_updated_at: new Date().toISOString() })
    .eq('id', key.id).eq('account_id', key.account_id)
  update = finalState === 'SENT' ? update.in('outside_state',['QUEUED','FAILED']) : update.eq('outside_state','QUEUED')

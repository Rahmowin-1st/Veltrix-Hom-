...
function baseBlockShape<T extends string>(type: T) {
  return { id: blockIdSchema, type: z.literal(type), version: z.literal(PART3_BLOCK_REGISTRY_VERSION) }
}

export const answerBlockSchema = z.object({ ...baseBlockShape('answer'), text: longTextSchema }).strict()
export const explanationBlockSchema = z.object({ ...baseBlockShape('explanation'), text: longTextSchema }).strict()
export const quoteBlockSchema = z.object({ ...baseBlockShape('quote'), text: longTextSchema, attribution: z.string().max(500).optional() }).strict()
...
export const checklistBlockSchema = z.object({
  ...baseBlockShape('checklist'),
  items: z.array(z.object({ id: blockIdSchema, text: z.string().min(1).max(4_000), checked: z.boolean().default(false) }).strict()).min(1).max(100),
}).strict()
...
export const tableBlockSchema = z.object({
  ...
  for (const [index, row] of value.rows.entries()) {
    if (row.length !== value.columns.length) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['rows', index], message: 'Every table row must match the column count.' })
  }
})
...
const mapGroupSchema = z.object({ id: blockIdSchema, label: z.string().min(1).max(500), nodeIds: z.array(blockIdSchema).max(300) }).strict()
const mapHierarchyLinkSchema = z.object({ parentId: blockIdSchema, childId: blockIdSchema }).strict()
...
export const mapBlockSchema = z.object({
  ...
  for (const [index, group] of value.groups.entries()) {
    ...
    for (const nodeId of group.nodeIds) if (!nodeIds.has(nodeId)) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['groups', index, 'nodeIds'], message: `Unknown map node: ${nodeId}` })
  }
...
export const timelineBlockSchema = z.object({
  ...baseBlockShape('timeline'),
  events: z.array(z.object({ id: blockIdSchema, label: z.string().min(1).max(1_000), timeLabel: z.string().min(1).max(500), description: z.string().max(4_000).optional() }).strict()).min(1).max(200),
}).strict()
...
const templateFieldSchema = z.object({
  ...
  if (value.fieldType === 'select' && (!value.options || value.options.length === 0)) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['options'], message: 'Select fields require bounded options.' })
  if (value.fieldType !== 'select' && value.options !== undefined) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['options'], message: 'Only select fields may define options.' })
})
...
const writingTextNodeSchema = z.object({ id: blockIdSchema, type: z.literal('paragraph'), text: z.string().max(20_000), marks: z.array(writingMarkSchema).max(24).default([]), ...writingCommonSchema }).strict()
const writingHeadingNodeSchema = z.object({ id: blockIdSchema, type: z.literal('heading'), level: z.union([z.literal(1), z.literal(2), z.literal(3)]), text: z.string().min(1).max(4_000), marks: z.array(writingMarkSchema).max(24).default([]), ...writingCommonSchema }).strict()
const writingQuoteNodeSchema = z.object({ id: blockIdSchema, type: z.literal('quote'), text: z.string().min(1).max(20_000), marks: z.array(writingMarkSchema).max(24).default([]), attribution: z.string().max(500).optional(), ...writingCommonSchema }).strict()
const writingListItemSchema = z.object({ id: blockIdSchema, text: z.string().min(1).max(8_000), marks: z.array(writingMarkSchema).max(24).default([]) }).strict()
const writingBulletListNodeSchema = z.object({ id: blockIdSchema, type: z.literal('bullet_list'), items: z.array(writingListItemSchema).min(1).max(100), ...writingCommonSchema }).strict()
const writingNumberedListNodeSchema = z.object({ id: blockIdSchema, type: z.literal('numbered_list'), items: z.array(writingListItemSchema).min(1).max(100), startAt: z.number().int().min(1).max(10_000).default(1), ...writingCommonSchema }).strict()
const writingChecklistNodeSchema = z.object({ id: blockIdSchema, type: z.literal('checklist'), items: z.array(writingListItemSchema.extend({ checked: z.boolean().default(false) }).strict()).min(1).max(100), ...writingCommonSchema }).strict()
const writingCodeNodeSchema = z.object({ id: blockIdSchema, type: z.literal('code_block'), code: z.string().max(PART3_BLOCK_MAX_TEXT), language: safeKeySchema.optional(), ...writingCommonSchema }).strict()
const writingDividerNodeSchema = z.object({ id: blockIdSchema, type: z.literal('divider'), treatment: z.enum(['plain', 'subtle', 'emphasis']).optional() }).strict()
...
const writingCollapsibleNodeSchema = z.object({ id: blockIdSchema, type: z.literal('collapse'), title: z.string().min(1).max(2_000), children: z.array(writingLeafNodeSchema).max(50), collapsedByDefault: z.boolean().default(true), ...writingCommonSchema }).strict()
const writingHiddenNodeSchema = z.object({ id: blockIdSchema, type: z.literal('hidden'), label: z.string().max(2_000).optional(), children: z.array(writingLeafNodeSchema).max(50), ...writingCommonSchema }).strict()
...
export const warningBlockSchema = z.object({ ...baseBlockShape('warning'), title: z.string().max(1_000).optional(), text: longTextSchema }).strict()
export const definitionBlockSchema = z.object({ ...baseBlockShape('definition'), term: z.string().min(1).max(1_000), definition: longTextSchema }).strict()
export const exampleBlockSchema = z.object({ ...baseBlockShape('example'), title: z.string().max(1_000).optional(), text: longTextSchema }).strict()
export const citationBlockSchema = z.object({
  ...baseBlockShape('citation'),
...
export const filePreviewBlockSchema = z.object({
  ...baseBlockShape('file_preview'),
  assetId: blockIdSchema,
  title: z.string().min(1).max(1_000),
  mimeType: z.string().min(1).max(255).optional(),
  previewKind: z.enum(['file', 'image', 'generated_artifact']).default('file'),
}).strict()
export const studioArtifactPreviewBlockSchema = z.object({ ...baseBlockShape('studio_artifact_preview'), placeholder: z.literal(true), title: z.string().max(1_000).optional() }).strict()

const interactiveOptionSchema = z.object({ id: blockIdSchema, text: z.string().min(1).max(4_000), isCorrect: z.boolean() }).strict()
export const interactiveTestBlockSchema = z.object({
  ...baseBlockShape('interactive_test'),
  ...
  for (const [index, question] of value.questions.entries()) {
    if (question.options.filter(option => option.isCorrect).length !== 1) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['questions', index, 'options'], message: 'Single-answer questions require exactly one correct option.' })
  }
})
...
function inspectInertJson(value: unknown, depth: number, seen: WeakSet<object>): InertJsonStats {
  ...
  if (typeof value === 'object') {
    if (seen.has(value)) throw new Error('Unknown block contains a cycle.')
...
export function parseContentBlocks(input: unknown): ContentBlock[] {
  ...
  if (Buffer.byteLength(JSON.stringify(blocks), 'utf8') > PART3_BLOCK_MAX_SERIALIZED_BYTES) throw new Error('Content block payload exceeds the persisted message limit.')
  return blocks
}

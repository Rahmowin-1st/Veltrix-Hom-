  ...
  try {
    ...
    try {
      ...
      const { data: cached } = await admin.rpc('match_answer_cache', {
        ...
        p_source_id: body.lockedSourceId ?? null,
    ...
    if (body.lockedSourceId) {
      sourceIds = [body.lockedSourceId]
      sourceMode = 'locked'
    ...
    if (questionEmbedding && parsed.confidence >= 0.6) {
      await admin.from('answer_cache').insert({
        user_id: userId,
        source_id: body.lockedSourceId ?? null,
        question,
...
chatRouter.get('/list', requireAuth, async (req, res, next) => {
  try {
    const { data } = await admin
      ...
      .eq('user_id', req.userId!).order('updated_at', { ascending: false }).limit(30)
    res.json({ chats: data ?? [] })
  } catch (e) { next(e) }
})

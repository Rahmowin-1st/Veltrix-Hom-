...
class FrontendQ2FinalEvidenceTest {
    ...
    private fun seedAdaptive() {
        PersonalizationStore(S.targetContext).save(AdaptiveState(recentToolIds = listOf("quadratic-solver", "vieta", "physics-ohms-law", "statistics-dataset", "finance-compound-interest")))
    }
    ...
    @Test fun bVisualToolMatrix() {
        ActivityScenario.launch(MainActivity::class.java).use { scenario ->
            ...
            openTool(scenario, "physics-ohms-law"); capture(scenario, "24_physics_ohms_law")
            ...
            openTool(scenario, "finance-compound-interest"); capture(scenario, "27_finance_compound_interest")
    ...
    @Test fun cVisualSystemMatrix() {
        ActivityScenario.launch(MainActivity::class.java).use { scenario ->
            ...
            captureWidget(scenario, "physics-ohms-law", "35_widget_formula")
        ...
        try {
            ActivityScenario.launch(MainActivity::class.java).use { scenario ->
                ...
                showScreen(scenario, "history"); setText(scenario, "history_search", "zzzz-no-history"); scenario.onActivity { S.invoke(it, "refreshHistoryScreen") }; Thread.sleep(160); capture(scenario, "46_empty_history_or_state")
            }
    ...
    private fun captureWidget(scenario: ActivityScenario<MainActivity>, toolId: String, name: String) {
        ...
        var available = false
        var unavailableReason = "AppWidget bind permission unavailable in emulator CI; runtime widget contracts are tested separately."
        scenario.onActivity { activity ->
            try {
                val manager = AppWidgetManager.getInstance(activity)
                val h = AppWidgetHost(activity, 9102).also { host = it; it.startListening() }
                widgetId = h.allocateAppWidgetId()
                val provider = ComponentName(activity, VeltrixToolWidgetProvider::class.java)
                available = manager.bindAppWidgetIdIfAllowed(widgetId, provider)
                if (available) {
                    WidgetConfigStore(activity).save(WidgetConfig(widgetId, toolId))
                    VeltrixToolWidgetProvider().onUpdate(activity, manager, intArrayOf(widgetId))
                    val info = manager.getAppWidgetInfo(widgetId)
                    val hostView = h.createView(activity, widgetId, info)
                    overlay = FrameLayout(activity).apply {
                        setBackgroundColor(Color.rgb(249,249,246))
                        addView(hostView, FrameLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, 560, Gravity.CENTER))
                    }
                ...
                available = false
                unavailableReason = "AppWidget host unavailable in emulator CI: ${t.javaClass.simpleName}: ${t.message}; runtime widget contracts are tested separately."
            }
        }
        Thread.sleep(180)
        if (available) S.screenshot(name) else S.writeText("visual/${name}_UNAVAILABLE.txt", unavailableReason)
        scenario.onActivity {
            overlay?.let { o -> (o.parent as? ViewGroup)?.removeView(o) }
            if (widgetId != AppWidgetManager.INVALID_APPWIDGET_ID) runCatching { host?.deleteAppWidgetId(widgetId) }

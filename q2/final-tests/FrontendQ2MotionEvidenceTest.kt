...
class FrontendQ2MotionEvidenceTest {
    ...
    @Test fun clip01_standard_press_rapid_equals() = launch { s ->
        ...
        S.injectTap(visible(s,"key_equals"),120); Thread.sleep(500)
    }
    ...
    @Test fun clip03_main_brain_icon_open_close() = launch { s ->
        var b:View?=null; s.onActivity { b=S.findByDescription(it.window.decorView,"Main Brain") }; S.injectTap(b!!,110); Thread.sleep(550)
        var c:View?=null; s.onActivity { c=S.findByDescription(it.window.decorView,"Close Main Brain") }; S.injectTap(c!!,100); Thread.sleep(450)
    }
    ...
    @Test fun clip06_library_custom_keyboard_biyt_vieta() = launch { s ->
        ...
        listOf("B","I","Y","T").forEach { ch -> var v:View?=null; s.onActivity { v=S.findByText(it.window.decorView,ch) }; S.injectTap(v!!,75); Thread.sleep(120) }; Thread.sleep(450)
    }
    @Test fun clip07_library_item_to_purpose_tool() = launch { s ->
        ...
        var v:View?=null; s.onActivity { v=S.clickableAncestor(S.findByText(it.window.decorView,"Vieta's Formulas")) }; S.injectTap(v!!,90); Thread.sleep(600)
    }
    ...
    @Test fun clip09_converter_swap() = launch { s ->
        s.onActivity { S.invoke(it,"showConverterEnvironment","length") }; Thread.sleep(260); var v:View?=null; s.onActivity { v=S.findByDescription(it.window.decorView,"Swap units") }; S.injectTap(v!!,130); Thread.sleep(500)
    }
    @Test fun clip10_currency_refresh_state() = launch { s ->
        ...
        s.onActivity { S.invoke(it,"showCurrencyEnvironment") }; Thread.sleep(400); var v:View?=null; s.onActivity { v=S.findByText(it.window.decorView,"Refresh rate") }; S.injectTap(v!!,90); Thread.sleep(900)
    }
    ...
    @Test fun clip12_slider_direct_manipulation() = launch { s ->
        show(s,"settings"); var slider:View?=null; s.onActivity { root -> slider=findFirstClass(root.window.decorView,"LiquidSlider") }; S.injectDrag(slider!!,0.18f,0.82f,durationMs=720); Thread.sleep(450)
    }
    @Test fun clip13_expanded_glass_control_space() = launch { s ->
        var b:View?=null; s.onActivity { b=S.findByDescription(it.window.decorView,"Main Brain") }; S.injectTap(b!!,110); Thread.sleep(850)
    }
    ...
    @Test fun clip17_reduced_motion_comparison() {
        ...
        try { launch { s -> var b:View?=null; s.onActivity { b=S.findByDescription(it.window.decorView,"Main Brain") }; S.injectTap(b!!,90); Thread.sleep(450) } }

...
export default function Sources() {
  ...
  useEffect(() => {
    setSources([]); setSubjects([]); setLoading(true)
    if (!userId) { setLoading(false); return }
    let cancelled = false
    Promise.all([sourceApi.list(), sourceApi.subjects()])
      .then(([a, b]) => { if (!cancelled) { setSources(a.sources); setSubjects(b.subjects) } })
      .catch(() => {})
  ...
  const reprocess = async (id: string) => {
    setSources((items) => items.map((s) => s.id === id ? { ...s, status: 'queued', progress: 0, error_message: null } : s))
    try {
      await sourceApi.reprocess(id)
      await refresh()
    } catch (e) {
      setSources((items) => items.map((s) => s.id === id ? { ...s, status: 'failed', error_message: e instanceof Error ? e.message : 'Qayta ishlashda xato.' } : s))
    }
  }
            ...
            <article key={s.id} id={`source-${s.id}`} data-highlight={params.get('source') === s.id ? '' : undefined} className="v5-source-card" style={{ '--source-color': s.color || '#0A6CFF', display: 'grid', gap: 'var(--s-3)' } as React.CSSProperties}>
              {motif && (
                <span aria-hidden style={{
                  ...
                  color: s.color, opacity: 0.075, letterSpacing: 7, lineHeight: 2.25,
              ...
              <div className="row" style={{ gap: 11, alignItems: 'flex-start', position: 'relative' }}>
                <span aria-hidden className="v5-source-icon" style={{ '--source-color': s.color || '#0A6CFF' } as React.CSSProperties}>{s.emoji}</span>
              ...
              {s.status === 'failed' && (
                <div role="alert" style={{ display: 'grid', gap: 8, position: 'relative' }}>
                  <p className="micro" style={{ color: 'var(--danger)' }}><AlertCircle size={12} style={{ verticalAlign: -2, marginRight: 4 }} />{s.error_message ?? 'Qayta ishlashda xato.'}</p>
                  <button className="btn btn-outline" style={{ height: 34 }} onClick={() => void reprocess(s.id)}><RefreshCw size={14}/> Qayta ishlash</button>
                </div>
              )}
              {ready && s.processing_warning && <p className="micro" style={{ color: 'var(--warning)', position: 'relative' }}><AlertCircle size={12} style={{ verticalAlign: -2, marginRight: 4 }}/>{s.processing_warning}</p>}
      ...
      <AnimatePresence>
        {adding && (
          <AddSourceFlow
            ...
            onClose={() => { setAdding(false); if (params.has('add')) { params.delete('add'); setParams(params, { replace: true }) } }}
    ...
    <BottomSheet title="Manbani tahrirlash" onClose={onClose}>
      <div style={{ display: 'grid', gap: 'var(--s-4)' }}>
        <label style={{ display: 'grid', gap: 6 }}>
          ...
          <input className="input" value={title} onChange={(e) => setTitle(e.target.value)} maxLength={15} />
        </label>

...
export default function Chat() {
  ...
  const send = useCallback(async (override?: string, overrideAttachment?: Attachment | null) => {
    ...
    if (skill?.instructions) prompt = `[SKILL: ${skill.name}] ${skill.instructions}\n\n${prompt}`
    ...
    try {
      const response = await api.sendMessage({
        ...
        image: sentAttachment ? { mimeType: sentAttachment.mimeType, data: sentAttachment.data } : null,
      }, controller.signal)
  ...
  }, [input, attachment, busy, translation, pickedSources, navigate, setDraft, upsertLocal, activeSkillId, skillById, consumeProject])
...
function ChatEmpty({ name }: { name: string | null }) {
  ...
  return <div style={{ minHeight: '52vh', display: 'grid', placeItems: 'center', alignContent: 'center', gap: 12, textAlign: 'center' }}>
    ...
    <p className="muted">Matn, ovoz, rasm, audio, fayl, manba va skill qo‘llanadi.</p>
  </div>
}

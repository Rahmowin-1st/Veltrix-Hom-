...
import {
  ...
  PanelLeftClose, PanelLeftOpen, Pin, MoreHorizontal, Plus, Library, Languages,
} from 'lucide-react'
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: `0 ${collapsed ? '10px' : 'var(--s-3)'}`, display: 'grid', gap: 4, flexShrink: 0 }}>
        ...
        <SideLink to="/tarjima" icon={<Languages size={18} />} label="Tarjima" collapsed={collapsed} onClick={close} />
        <SideLink to="/manbalar" icon={<Library size={18} />} label="Manbalar" collapsed={collapsed} onClick={close} />
        <SideLink to="/skills" icon={<Sparkles size={18} />} label="Skills" collapsed={collapsed} onClick={close} />
      ...
      >
        ...
        <Section label="Loyihalar" action={
          ...
          {projects.map((pr) => (
            <button key={pr.id} onClick={() => { navigate(`/loyiha/${pr.id}`); close() }}
              title={pr.name}
              style={{
